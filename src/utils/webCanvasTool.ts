...
export async function extractFirstFrameEdges(
  ...
  try {
    ...
    if (!frameWidth || !frameHeight) {
      const { frameWidth: detectedWidth, frameHeight: detectedHeight } = detectSpriteSheetFrames(
        imageData,
        canvasWidth,
        canvasHeight
        // canvas.width,
    ...
    const verticesArr = detectEdgesByErosionDilation(frameData, frameWidth, frameHeight)
...
function detectSpriteSheetFrames(
  ...
  const horizontalAvg = horizontalDistribution.reduce((sum, count) => sum + count, 0) / width
  const verticalAvg = verticalDistribution.reduce((sum, count) => sum + count, 0) / height
  const horizontalDividers = findDividers(horizontalDistribution, horizontalAvg * 0.2)
  const verticalDividers = findDividers(verticalDistribution, verticalAvg * 0.2)
...
function findDividers(distribution: number[], threshold: number): number[] {
  ...
  for (const pos of dividers) {
    if (currentGroup.length === 0 || pos - currentGroup[currentGroup.length - 1] < mergeThreshold) {
      currentGroup.push(pos)
    } else {
      // 取组内中间位置作为分隔线
      mergedDividers.push(
        Math.round(currentGroup.reduce((sum, p) => sum + p, 0) / currentGroup.length)
      )
      currentGroup = [pos]
    }
  }
...
export function calculateSpriteSheetFrames(
  ...
  // 参数校验
  if (wholeWidth <= 0 || wholeHeight <= 0 || frameWidthEstimate <= 0 || frameHeightEstimate <= 0) {
  ...
  for (let cols = minCols; cols <= maxCols; cols++) {
    ...
    for (let rows = minRows; rows <= maxRows; rows++) {
      ...
      const widthInRange =
        frameWidth >= frameWidthEstimate * 0.8 && frameWidth <= frameWidthEstimate * 1.2
      const heightInRange =
        frameHeight >= frameHeightEstimate * 0.8 && frameHeight <= frameHeightEstimate * 1.2

      // 检查是否能整除（无余数）
      const exactFit = Number.isInteger(wholeWidth / cols) && Number.isInteger(wholeHeight / rows)
  ...
  const bestConfig = validConfigs.reduce((best, current) => {
    const currentError =
      Math.pow((current.frameWidth - frameWidthEstimate) / frameWidthEstimate, 2) +
      Math.pow((current.frameHeight - frameHeightEstimate) / frameHeightEstimate, 2)

    const bestError =
      Math.pow((best.frameWidth - frameWidthEstimate) / frameWidthEstimate, 2) +
      Math.pow((best.frameHeight - frameHeightEstimate) / frameHeightEstimate, 2)
